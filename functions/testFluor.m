function [cellsOfInterest, stalkedIdx, stalked] = testFluor(phase)
...
% Per cell visualization (code from
% https://www.mathworks.com/help/matlab/ref/inpolygon.html)
%{
figure
plot(Xperim,Yperim) % polygon
axis equal

hold on
plot(J(in),I(in),'r+') % points inside
plot(J(~in),I(~in),'bo') % points outside
if (iCell == numCells)
    hold off
end
%}

    % test for multiple fluorescence in cell
    % reject cell if multiple fluorescence with too much space in between
    all_I = I(find(in == 1));
    all_J = J(find(in == 1));
    ...
    % fluorcells stores boolean for each cell - 1=hasFluor, 0=doesn't
